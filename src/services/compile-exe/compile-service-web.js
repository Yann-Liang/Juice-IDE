...
class compileServies {
    ...
    compiler(path = 'src/contract/FileServerManager.sol'){
        ...
        _this.grammarCheck(function(result, missingInputs, source){
            ...
            if(JSON.stringify(result.contracts) == "{}"){
                store.dispatch('compileWatch',3);
                consoleService.output({logError:'Compiler Failed'});
                consoleService.output(falseData);
            }else{
                var compileData=[]
                for(var key in result.contracts){
                    compileData.push({
                        contractName:key,
                        abi:result.contracts[key].interface,
                        bin:result.contracts[key].bytecode
                    })
                }
                falseData.data = compileData;
