...
import "./library/LibFileServer.sol";
import "./sysbase/OwnerNamed.sol";
import "./interfaces/IFileServerManager.sol";

/** FileServerManager contract */
contract FileServerManager is OwnerNamed, IFileServerManager {
	using LibString for *;
    using LibInt for *;
    ...
    function FileServerManager() {
    	register("SystemModuleManager","0.0.1.0","FileServerManager", "0.0.1.0");
    }
    ...
    */
    ...
    function find(string _serverId) constant public returns(string _ret) {
        ...
        if (fileServerInfoMap[_serverId].state != LibFileServer.ServerState.INVALID) {
            count = 1;
            len = LibStack.appendKeyValue("total", count);
            len = LibStack.append(",\"items\":[");
            len = LibStack.append(fileServerInfoMap[_serverId].toJson());
        } else {
            len = LibStack.appendKeyValue("total", count);
            len = LibStack.append(",\"items\":[");
        }
    ...
    function listAll() constant public returns(string _ret){
        ...
        len = LibStack.appendKeyValue("ret", uint(0));
        len = LibStack.append(",\"data\":{");
        len = LibStack.appendKeyValue("total", fileServerIdList.length);
        len = LibStack.append(",\"items\":[");
        ...
        for (uint index = 0; index < fileServerIdList.length; index++) {
            if (fileServerInfoMap[fileServerIdList[index]].state != LibFileServer.ServerState.INVALID) {
                ...
                total = total + 1;
                len = LibStack.append(fileServerInfoMap[fileServerIdList[index]].toJson());
            }
        }
        len = LibStack.append("]}}");
        _ret = LibStack.popex(len);
