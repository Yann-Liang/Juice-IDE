...
import "./library/LibFile.sol";
import "./interfaces/IFileInfoManager.sol";

contract FileInfoManager is OwnerNamed ,IFileInfoManager {
    using LibString for *;
    using LibInt for *;
    using LibFile for *;
    ...
    function FileInfoManager() {
       	register("SystemModuleManager","0.0.1.0","FileInfoManager", "0.0.1.0");
        defaultPageSize = 16;
    }
    ...
    */
    ...
    function find(string _fileId) constant public returns(string _ret) {
        ...
        if (fileInfoMap[_fileId].state != LibFile.FileState.FILE_INVALID) {
            ...
            len = LibStack.append(fileInfoMap[_fileId].toJson());
        }else{
            len = LibStack.appendKeyValue("total", count);
            len = LibStack.append(",\"items\":[");
        } 
    ...
    function listAll() constant public returns (string _ret){
        ...
        len = LibStack.appendKeyValue("ret", uint(0));
        len = LibStack.append(",\"data\":{");
        len = LibStack.appendKeyValue("total", fileIdList.length);
        len = LibStack.append(",\"items\":[");
        ...
        for (uint index = 0; index < fileIdList.length; index++) {
            if (fileInfoMap[fileIdList[index]].state != LibFile.FileState.FILE_INVALID) {
                ...
                len = LibStack.append(fileInfoMap[fileIdList[index]].toJson());
